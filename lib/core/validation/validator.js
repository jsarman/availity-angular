(function(root) {
  ...
  var addValidators = function(existingValidators, newValidators) {
    newValidators = _.isArray(newValidators) ? newValidators : [newValidators];
    _.forEach(newValidators, function(validator) {
      if (validator.name) {
        existingValidators[validator.name] = validator;
      }
    });
  };
  ...
  availity.core.provider('avVal', function() {
    ...
    this.addValidators = function(validators) {
      addValidators(this.validators, validators);
    };
