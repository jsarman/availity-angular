(function(root) {
  ...
  availity.ui.factory('avValBootstrapAdapter', function(AV_BOOTSTRAP_ADAPTER, $timeout, $log) {
    ...
    return {
      ...
      group: function(element, ngModel) {
        if(ngModel.$valid) {
          element.children(AV_BOOTSTRAP_ADAPTER.CLASSES.FORM_GROUP_ALL).removeClass(AV_BOOTSTRAP_ADAPTER.CLASSES.WARNING);
        }else {
          element.children(AV_BOOTSTRAP_ADAPTER.CLASSES.FORM_GROUP_ALL).addClass(AV_BOOTSTRAP_ADAPTER.CLASSES.WARNING);
        }
      },
