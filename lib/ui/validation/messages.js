(function(root) {
  ...
  availity.ui.controller('avValContainerController', function($scope, $timeout) {
    ...
    this.message = function(ngModel) {
      ...
      var message = null;
      // need a way to show message on load either global using form attribute or field
      if(ngModel.avResults.violations.length && ngModel.avResults.violations[0].message && ngModel.$dirty) {
